/// \file LiquidCrystal_PCF8574_SoftWire.cpp
/// \brief LiquidCrystal library with PCF8574 SoftWire adapter.

/// This is a library for driving LiquidCrystal displays (LCD) by using the SoftWire-I2C bus 
/// and an PCF8574 I2C adapter.
/// \author Matthias Hertel, http://www.mathertel.de
/// \copyright Copyright (c) 2019 by Matthias Hertel.
/// ported to SoftWire from: Jürgen Schnaiter
/// \copyright Copyright (c) 2025 by Jürgen Schnaiter.
/// ChangeLog see: LiquidCrystal_PCF8574_SoftWire.h

#include "LiquidCrystal_PCF8574_SoftWire.h"
#include <SoftWire.h>

LiquidCrystal_PCF8574_SoftWire::LiquidCrystal_PCF8574_SoftWire(uint8_t i2cAddr)
{
...
LiquidCrystal_PCF8574_SoftWire::LiquidCrystal_PCF8574_SoftWire(uint8_t i2cAddr, uint8_t rs, uint8_t enable,
    uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7, uint8_t backlight)
{
  init(i2cAddr, rs, 255, enable, d4, d5, d6, d7, backlight);
} // LiquidCrystal_PCF8574

LiquidCrystal_PCF8574_SoftWire::LiquidCrystal_PCF8574_SoftWire(uint8_t i2cAddr, uint8_t rs, uint8_t rw, uint8_t enable,
    uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7, uint8_t backlight)
{
  init(i2cAddr, rs, rw, enable, d4, d5, d6, d7, backlight);
} // LiquidCrystal_PCF8574

void LiquidCrystal_PCF8574_SoftWire::init(uint8_t i2cAddr, uint8_t rs, uint8_t rw, uint8_t enable,
    uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7, uint8_t backlight)
{
  _i2cAddr = i2cAddr;
  _backlight = 0;
...
void LiquidCrystal_PCF8574_SoftWire::begin(uint8_t cols, uint8_t lines, SoftWire &wirePort)
{
...
void LiquidCrystal_PCF8574_SoftWire::clear()
{
...
void LiquidCrystal_PCF8574_SoftWire::home()
{
...
void LiquidCrystal_PCF8574_SoftWire::setCursor(uint8_t col, uint8_t row)
{
  // check boundaries
...
void LiquidCrystal_PCF8574_SoftWire::noDisplay()
{
...
void LiquidCrystal_PCF8574_SoftWire::display()
{
  // Instruction: Display on/off control = 0x08
  _displaycontrol |= 0x04; // display
  _send(0x08 | _displaycontrol);
...
void LiquidCrystal_PCF8574_SoftWire::cursor()
{
  // Instruction: Display on/off control = 0x08
...
void LiquidCrystal_PCF8574_SoftWire::noCursor()
{
  // Instruction: Display on/off control = 0x08
  _displaycontrol &= ~0x02; // cursor
  _send(0x08 | _displaycontrol);
...
void LiquidCrystal_PCF8574_SoftWire::blink()
{
  // Instruction: Display on/off control = 0x08
...
void LiquidCrystal_PCF8574_SoftWire::noBlink()
{
  // Instruction: Display on/off control = 0x08
  _displaycontrol &= ~0x01; // blink
  _send(0x08 | _displaycontrol);
...
void LiquidCrystal_PCF8574_SoftWire::scrollDisplayLeft(void)
{
...
void LiquidCrystal_PCF8574_SoftWire::scrollDisplayRight(void)
{
  // Instruction: Cursor or display shift = 0x10
...
void LiquidCrystal_PCF8574_SoftWire::leftToRight(void)
{
...
void LiquidCrystal_PCF8574_SoftWire::rightToLeft(void)
{
...
void LiquidCrystal_PCF8574_SoftWire::autoscroll(void)
{
...
void LiquidCrystal_PCF8574_SoftWire::noAutoscroll(void)
{
...
void LiquidCrystal_PCF8574_SoftWire::setBacklight(uint8_t brightness)
{
...
void LiquidCrystal_PCF8574_SoftWire::createChar(uint8_t location, uint8_t charmap[])
{
...
inline size_t LiquidCrystal_PCF8574_SoftWire::write(uint8_t ch)
{
  _send(ch, true);
...
void LiquidCrystal_PCF8574_SoftWire::_send(uint8_t value, bool isData)
{
...
void LiquidCrystal_PCF8574_SoftWire::_writeNibble(uint8_t halfByte, bool isData)
{
  ...
  _i2cPort->llWrite(data | _enable_mask);
  // delayMicroseconds(1); // enable pulse must be >450ns
  _i2cPort->llWrite(data);
  // delayMicroseconds(37); // commands need > 37us to settle
} // _writeNibble


// write a nibble / halfByte with handshake
void LiquidCrystal_PCF8574_SoftWire::_sendNibble(uint8_t halfByte, bool isData)
{
  _i2cPort->llStart(_i2cAddr<<1);
...
void LiquidCrystal_PCF8574_SoftWire::_write2Wire(uint8_t data, bool isData, bool enable)
{
  if (isData)
    data |= _rs_mask;
  // _rw_mask is not used here.
